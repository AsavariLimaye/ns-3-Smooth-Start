...
TcpSocketState::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpSocketState")
    ...
    .AddTraceSource ("NextTxSequence",
                     ...
                     "ns3::SequenceNumber32TracedValueCallback")
    .AddTraceSource ("SmoothStartThreshold",
                     "TCP smooth start threshold (bytes)",
                     MakeTraceSourceAccessor (&TcpSocketState::m_smsThresh),
                     "ns3::TracedValue::Uint32Callback")
    .AddAttribute ("GrainNumber",
                   "Grain number used for Smooth Start",
                   UintegerValue (2),
                   MakeUintegerAccessor (&TcpSocketState::m_grainNumber),
                   MakeUintegerChecker<uint16_t> ()
                   )
    .AddAttribute ("Depth",
                   "Depth used for Smooth Start",
                   UintegerValue (1),
                   MakeUintegerAccessor (&TcpSocketState::m_depth),
                   MakeUintegerChecker<uint16_t> ()
                   )  
     .AddAttribute ("isSmoothStart", "Enable Smooth Start",
                   BooleanValue (false),
                   MakeBooleanAccessor (&TcpSocketState::m_isSmoothStart),
                   MakeBooleanChecker ())
...
TcpSocketState::TcpSocketState (const TcpSocketState &other)
  : Object (other),
    ...
    m_isSmoothStart (other.m_isSmoothStart),
...
TcpSocketBase::SetInitialSSThresh (uint32_t threshold)
{
  ...
  m_tcb->m_smsThresh = m_tcb->m_initialSsThresh / pow(2,m_tcb->m_depth);
  m_tcb->m_ackThresh = m_tcb->m_grainNumber;
}
